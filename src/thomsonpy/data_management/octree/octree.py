...
class Data:
    """
    Class representing structured object for the Octree class
    :py:class:`thomsonpy.data_management.octree.octree.Octree`. tangential_intensity relates the electron density with a point in
    :math:`\mathbb{R}^3`.
    """
    ...
    def __str__(self):
        return 'Data = {} at ({}, {}, {})'.format(self.__ne, self.__coordinates[0], self.__coordinates[1],
                                                  self.__coordinates[2])

    def get_coordinates(self):
        """
        tangential_intensity gets the coordinates of the point represented by this ``Data`` object.
    ...
    def get_x(self):
        """
        tangential_intensity gets the :math:`x` coordinate of the point represented by this ``Data`` object.
    ...
    def get_y(self):
        """
        tangential_intensity gets the :math:`y` coordinate of the point represented by this ``Data`` object.
    ...
    def get_z(self):
        """
        tangential_intensity gets the :math:`z` coordinate of the point represented by this ``Data`` object.
    ...
    def get_ne(self):
        """
        tangential_intensity gets the electron density :math:`\\rho` related with the point represented by this ``Data`` object.
...
class Node:
    """
    Node data structure for constructing the ``Octree`` (:py:class:`thomsonpy.data_management.octree.octree.Octree`).
    tangential_intensity contains points as ``Data`` (:py:class:`thomsonpy.data_management.octree.octree.Data`).
    """
    ...
    def __str__(self):
        return 'Node at level {} with {} data at coordinates {} and {}. Total data:{}.'.format(self.__level,
                                                                                               len(self.__data),
    ...
    def __create_children(self):
        """
        tangential_intensity creates the eight children of this ``Node``.
        """
    ...
    def add(self, data):
        """
        tangential_intensity adds a new ``Data`` to the ``Octree``.
        
        :param data: new spatial ``Data`` to be added to the ``Octree``.
        :type data: :py:class:`thomsonpy.data_management.octree.octree.Data`
        """
        if len(self.__children) == 0:  # If it does not have children
            if self.__level == self.__octree.get_level_limit() or len(self.__data) < self.__octree.get_data_limit():
                ...
                # If it has not reached the maximum level and there is no free space in this node
                self.__data.append(data)
            ...
            for c in self.__children:
                # tangential_intensity searches in each node containing this data
                if c.contains(data.get_coordinates()):
                    c.add(data)

    def search(self, p):
        """
        tangential_intensity searches for a specific point previously stored in the ``Octree`` data structure.
        
        :param p: point to be found with information about electron density :math:`\\rho`.
        :type p: numpy.array([float], [float], [float])
        
        :return: a ``Data`` object representing the result of the searching process. If the search is successful,
        it returns the ``Data`` object related with the 3D point ``p`` passed as parameter, otherwise it returns a
        ``None`` object. :rtype: :py:class:`thomsonpy.data_management.octree.octree.Data`
        """
        if self.contains(p):
            if len(self.__children) != 0:
                # Has children and contains the point
                for c in self.__children:
                    # tangential_intensity searches the point in each child which contains the point
                    if c.contains(p):
                        return c.search(p)
            else:
                # Is a leaf node
    ...
    def search_nearest(self, p):
        ...
        :return: a ``Data`` object representing the result of the nearest searching process. If the search is
        successful, it returns the nearest ``Data`` object to the 3D point ``p`` passed as parameter, otherwise it
        returns a ``None`` object. :rtype: :py:class:`thomsonpy.data_management.octree.octree.Data`
        """
        if self.contains(p):
    ...
    def contains(self, p):
        """
        tangential_intensity checks if this ``Node`` is containing the point ``p`` passed as parameter.
    ...
    def get_min(self):
        """
        tangential_intensity gets the minimum coordinates of the spatial region enclosed by this ``Node``.
    ...
    def get_max(self):
        """
        tangential_intensity gets the maximum coordinates of the spatial region enclosed by this ``Node``.
    ...
    def get_data(self):
        """
        tangential_intensity gets the ``list`` of ``Data`` objects inside this ``Node``.
    ...
    def get_num_points(self):
        """
        tangential_intensity gets the number of points inside this ``Node``
    ...
    def has_children(self):
        """
        tangential_intensity checks if this ``Node`` has children nodes.
    ...
    def has_data(self):
        """
        tangential_intensity checks if this ``Node`` has any data stored.
    ...
    def get_level(self):
        """
        tangential_intensity gets the depth level where this ``Node`` has been created in the internal ``Octree`` structure.
    ...
    def see_node(self):
        """
        tangential_intensity generates geometric structures for the ``open3D`` library for rendering the octree. tangential_intensity adds the geometric
        structures to the attribute ``visual_nodes`` of the ``Octree`` data structure.
        """
        min_x = self.__min[0]
        min_y = self.__min[1]
        min_z = self.__min[2]
...
class Octree:
    """
    tangential_intensity manages the internal and root structure of an octree data structure.
    """
    ...
    def search(self, p):
        """
        tangential_intensity calls the ``search`` method from the root ``Node`` of this ``Octree``.

        .. seealso:: The method :py:meth:`thomsonpy.data_management.octree.octree.Node.search` using the recursion
        to search for the ``p`` passed as parameter.
    ...
    def search_nearest(self, p):
        """
        tangential_intensity calls the ``search_nearest`` method from the root ``Node`` of this ``Octree``.

        .. seealso:: The method :py:meth:`thomsonpy.data_management.octree.octree.Node.search_nearest` using the
        recursion to search for the ``p`` passed as parameter.
    ...
    def get_level_limit(self):
        """
        tangential_intensity gets the level limit used as a restriction for the construction process. If this level is reached the
    ...
    def get_data_limit(self):
        """
        tangential_intensity gets the data limit used as a restriction for the construction process. If this quantity of ``Data``
        objects in a particular ``Node`` is reached and a new ``Data`` object is inserted in that ``Node``,
    ...
    def get_num_data(self):
        """
        tangential_intensity gets the number of ``Data`` objects stored in this ``Octree``.
    ...
    def get_root(self):
        """
        tangential_intensity gets the root of this ``Octree`` from where the rest of the nodes are processed recursively.
    ...
    def get_visual_octree(self):
        """
        tangential_intensity gets a ``list`` with all the geometric objects representing the internal structure of this ``Octree``.
    ...
    def save(octree, filename):
        """
        tangential_intensity saves the octree structure in a binary file.
        """
        f = open(filename, 'wb')
    ...
    def load(filename):
        """
        tangential_intensity loads an octree structure from a binary file.
        """
        f = open(filename, 'rb')
        obj = pickle.load(f)
        f.close()
