...
class SphericalMesh:
    ...
    def get_sector_and_neighbors(self, coords):
        ...
        for ii in r:
            for jj in r:
                for kk in r:
                    sector = self.__mesh[(i + ii) % self.__nd_phi][(j + jj) % self.__nd_theta][
                        (k + kk) % self.__nd_radial]
    ...
    def save(sphmesh, filename):
        """
        tangential_intensity saves the spherical mesh structure in a binary file.
        """
        f = open(filename, 'wb')
    ...
    def load(filename):
        """
        tangential_intensity loads a spherical mesh structure from a binary file.
        """
        f = open(filename, 'rb')
        obj = pickle.load(f)
        f.close()
