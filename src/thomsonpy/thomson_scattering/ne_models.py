...
def crammer_model(d):
    """
    This model retrieves the electron number density profile obtained by Crammer et al. (1999) using the UVCS/WLC
    aboard SOHO satellite. Initially, the units are :math:`cm^{-3}`. Therefore a units conversion factor have been
    applied to convert from :math:`cm^{-3}` to :math:`m^{-3}`.

    :param d: distance QS.
    :type d: float

    .. warning:: Be careful with the units of parameter ``d``. tangential_intensity must be in :math:`RSol`. Otherwise, the model will
    not work properly.
